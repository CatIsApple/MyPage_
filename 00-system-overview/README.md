<sub><a href="../">MyPage_</a> / 00 . System Overview (시스템 전체 흐름)</sub>

# 00 . System Overview (시스템 전체 흐름)

<a href="#subcategories-하위-카테고리"><img src="../assets/categories/00-system-overview.png" alt="00 . System Overview (시스템 전체 흐름)" width="100%" align="top"></a><a href="#subcategories-하위-카테고리"><img src="../assets/ui/view-category.png" alt="카테고리 보기" width="100%" align="top"></a>

<sub>6 SUBCATEGORIES (하위 카테고리) · 3 ARTICLES (글)</sub>

## Subcategories (하위 카테고리)

<table>
  <tr>
    <td valign="middle">
      <sub>SUBCATEGORY (하위 카테고리) 01 · 1 ARTICLE (글)</sub><br>
      <strong>System Components (시스템 구성 요소)</strong><br><br>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="bottom">
      <strong><a href="./01-system-components/">View Category (카테고리 보기) →</a></strong>
    </td>
  </tr>
</table>

<table>
  <tr>
    <td valign="middle">
      <sub>SUBCATEGORY (하위 카테고리) 02 · 2 ARTICLES (글)</sub><br>
      <strong>Program Execution Flow (프로그램 실행 전체 흐름)</strong><br><br>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="bottom">
      <strong><a href="./02-program-execution-flow/">View Category (카테고리 보기) →</a></strong>
    </td>
  </tr>
</table>

<table>
  <tr>
    <td valign="middle">
      <sub>SUBCATEGORY (하위 카테고리) 03 · 0 ARTICLES (글)</sub><br>
      <strong>Web Request Flow (웹 요청 전체 흐름)</strong><br><br>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="bottom">
      <strong><a href="./03-web-request-flow/">View Category (카테고리 보기) →</a></strong>
    </td>
  </tr>
</table>

<table>
  <tr>
    <td valign="middle">
      <sub>SUBCATEGORY (하위 카테고리) 04 · 0 ARTICLES (글)</sub><br>
      <strong>File I/O Flow (파일 입출력 전체 흐름)</strong><br><br>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="bottom">
      <strong><a href="./04-file-io-flow/">View Category (카테고리 보기) →</a></strong>
    </td>
  </tr>
</table>

<table>
  <tr>
    <td valign="middle">
      <sub>SUBCATEGORY (하위 카테고리) 05 · 0 ARTICLES (글)</sub><br>
      <strong>Concurrent Execution Flow (동시 실행 전체 흐름)</strong><br><br>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="bottom">
      <strong><a href="./05-concurrent-execution-flow/">View Category (카테고리 보기) →</a></strong>
    </td>
  </tr>
</table>

<table>
  <tr>
    <td valign="middle">
      <sub>SUBCATEGORY (하위 카테고리) 06 · 0 ARTICLES (글)</sub><br>
      <strong>Failure &amp; Recovery Flow (장애와 복구 전체 흐름)</strong><br><br>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="bottom">
      <strong><a href="./06-failure-and-recovery-flow/">View Category (카테고리 보기) →</a></strong>
    </td>
  </tr>
</table>

## Latest in This Category (이 카테고리의 최신 글)

<table>
  <tr>
    <td rowspan="2" width="40%" valign="top">
      <a href="./01-system-components/runtime/">
        <img src="../assets/articles/runtime/cover.jpg" alt="Runtime (런타임)" width="100%">
      </a>
    </td>
    <td width="60%" valign="top">
      <sub>SYSTEM COMPONENTS (시스템 구성 요소) · 2026.09.07</sub><br>
      <strong>Runtime (런타임)</strong><br>
      컴파일과 링크 이후 프로그램이 실행되는 런타임을 중심으로 런타임 환경·시스템·라이브러리의 역할과 차이를 정리했다.
    </td>
  </tr>
  <tr height="1">
    <td width="60%" height="1" valign="bottom">
      <strong><a href="./01-system-components/runtime/">Read Article (글 읽기) →</a></strong>&nbsp;&nbsp;·&nbsp;&nbsp;
      <strong><a href="https://catisapple.github.io/MyPage_/00-system-overview/01-system-components/runtime/">Read on Pages (글만 보기) ↗</a></strong>
    </td>
  </tr>
</table>

---

[← Back to MyPage_ (MyPage_로 돌아가기)](../)
